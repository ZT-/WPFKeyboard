...
CKLL::CKLL(void) : pfnKbdLayerDescriptor(nullptr)
{
	hHandle = NULL;
	KbdTables = NULL;
	KbdTables64 = NULL;
...
BOOL CKLL::LoadDLL(wchar_t* sKeyboardDll)
{
	//Unload if loaded...
	if (hHandle)
		this->UnloadDLL();
...
{
	for (size_t i = 0; i < m_vkarray.size(); i++)
	{
...
{
	for (size_t i = 0; i < m_vkModifiersArray.size(); i++)
	{
...
{
	for (size_t i = 0; i < m_vkScanCodesArray.size(); i++)
	{
...
{
	for (size_t i = 0; i < m_scTextArray.size(); i++)
	{
